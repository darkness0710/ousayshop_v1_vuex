...
$(document).ready(function () {
  ...
  }), $('.selectpicker').length != 0 && $('.selectpicker').selectpicker(), $('[data-toggle="popover"]').popover(), $('.carousel').carousel({
    interval: 3e3
  })
  var a = $('.tagsinput').data('color')
  $('.tagsinput').tagsinput(), $('.bootstrap-tagsinput').addClass('' + a + '-badge'), $('.navbar-color-on-scroll').length != 0 && $(window).on('scroll', materialKit.checkScrollForTransparentNavbar), materialKit.checkScrollForTransparentNavbar(), window_width >= 768 && (big_image = $('.page-header[data-parallax="true"]'), big_image.length != 0 && $(window).on('scroll', materialKitDemo.checkScrollForParallax))
  ...
  $toggle = $(this), materialKit.misc.navbar_menu_visible == 1 ? ($('html').removeClass('nav-open'), materialKit.misc.navbar_menu_visible = 0, $('#bodyClick').remove(), setTimeout(function () {
  ...
  initColoredShadows: function () {
    !0 != materialKit.misc.colored_shadows || BrowserDetect.browser == 'Explorer' && BrowserDetect.version <= 12 || $('.card:not([data-colored-shadow="false"]) .card-header-image').each(function () {
      if ($card_img = $(this), is_on_dark_screen = $(this).closest('.section-dark, .section-image').length, is_on_dark_screen == 0) {
        var a = $card_img.find('img').attr('src')
        var b = $card_img.closest('.card-rotate').length == 1
        var c = $card_img
        var d = $('<div class="colored-shadow"/>')
        if (b) {
          var e = $card_img.height()
          var f = $card_img.width()
          $(this).find('.back').css({
            height: e + 'px',
            width: f + 'px'
        ...
        }).appendTo(c), $card_img.width() > 700 && d.addClass('colored-shadow-big'), setTimeout(function () {
          d.css('opacity', 1)
        }, 200)
      }
    })
  ...
  initRotateCard: debounce(function () {
    $('.rotating-card-container .card-rotate').each(function () {
      ...
      var c = $(this).find('.front .card-body').outerHeight()
      a.parent().css({
        height: c + 'px',
        'margin-bottom': '30px'
      }), a.find('.front').css({
...
var BrowserDetect = {
  ...
  searchString: function (a) {
    for (var b, c = 0; c < a.length; c++) { if (b = a[c].string, this.versionSearchString = a[c].subString, b.indexOf(a[c].subString) !== -1) return a[c].identity }
  },
  searchVersion: function (a) {
    var b = a.indexOf(this.versionSearchString)
    if (b !== -1) {
      var c = a.indexOf('rv:')
      return this.versionSearchString === 'Trident' && c !== -1 ? parseFloat(a.substring(c + 3)) : parseFloat(a.substring(b + this.versionSearchString.length + 1))
    }
  },
  dataBrowser: [{
